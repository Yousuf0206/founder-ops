...
export default async function DraftPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="max-w-3xl">
      <Link href="/content" className="text-sm text-[--color-muted]">
        ← Content
      </Link>
    ...
    <div className="mt-6 flex flex-col gap-4">
      ...
      {payload.hashtags.length > 0 && (
        <section>
          ...
          <p className="mt-1 text-sm text-[--color-muted]">{payload.hashtags.join(" ")}</p>
        </section>
      )}
    </div>
  );
...
function ListBlock({ title, items }: { title: string; items: string[] }) {
  ...
  return (
    <section>
      <h2 className="text-sm font-medium">{title}</h2>
      <ul className="mt-1 list-inside list-disc rounded-lg border border-[--color-line] bg-[--color-surface] p-3 text-sm">
